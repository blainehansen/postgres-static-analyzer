...
select
	...
	pg_get_userbyid(pg_database.datdba)::text as datdba, -- oid (references pg_authid.oid) Owner of the database, usually the user who created it
	...
	pg_database.datlocprovider as datlocprovider, -- char  Locale provider for this database: b = builtin, c = libc, i = icu
	pg_database.datistemplate as datistemplate, -- bool  If true, then this database can be cloned by any user with CREATEDB privileges; if false, then only superusers or the owner of the database can clone it.
	pg_database.datallowconn as datallowconn, -- bool  If false then no one can connect to this database. This is used to protect the template0 database from being altered.
	...
	pg_database.datcollate as datcollate, -- text  LC_COLLATE for this database
