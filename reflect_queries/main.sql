...
from
	...
	cross join lateral aclexplode(nspacl) with ordinality as a
...
--! reflect_type_grants
select
	typ.typname::text, sch.nspname::text,
	array_agg(case when grantee = 0 then 'public' else pg_get_userbyid(grantee)::text end order by a.ordinality) as grantees,
	-- array_agg(privilege_type order by a.ordinality) as privilege_types,
	array_agg(is_grantable order by a.ordinality) as is_grantables,
	array_agg(pg_get_userbyid(grantor)::text order by a.ordinality) as grantors
from
	pg_catalog.pg_type as typ
	join pg_catalog.pg_namespace as sch on typ.typnamespace = sch.oid
	cross join lateral aclexplode(typ.typacl) with ordinality as a
where sch.nspname not in ('pg_catalog', 'information_schema', 'pg_toast')
group by typ.oid, sch.oid
;


--! reflect_table_grants
select
	tab.relname::text, sch.nspname::text,
	array_agg(case when grantee = 0 then 'public' else pg_get_userbyid(grantee)::text end order by a.ordinality) as grantees,
	array_agg(privilege_type order by a.ordinality) as privilege_types,
	array_agg(is_grantable order by a.ordinality) as is_grantables,
	array_agg(pg_get_userbyid(grantor)::text order by a.ordinality) as grantors
from
	pg_catalog.pg_class as tab
	join pg_catalog.pg_namespace as sch on tab.relnamespace = sch.oid
	cross join lateral aclexplode(tab.relacl) with ordinality as a
where
	tab.relkind = 'r'
	and sch.nspname not in ('pg_catalog', 'information_schema', 'pg_toast')
